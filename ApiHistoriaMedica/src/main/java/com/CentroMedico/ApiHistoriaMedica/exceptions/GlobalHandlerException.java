...
public class GlobalHandlerException {
    ...
    public ResponseEntity<Object> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        // Recoge los errores de validación de los campos
    ...
    /**
     * Maneja las excepciones de argumento ilegal (ej. DNI no existe, Historia ya existe, ID no encontrado).
    ...
    /**
     * Maneja las excepciones de estado ilegal (ej. Servicio dependiente no disponible/estado incorrecto).
     * Devuelve HttpStatus 503 SERVICE_UNAVAILABLE (o 500 si no quieres exponer 503).
     */
    @ExceptionHandler(IllegalStateException.class)
    public ResponseEntity<Map<String, Object>> handleIllegalStateException(IllegalStateException ex) {
        // Usamos 503 SERVICE_UNAVAILABLE para indicar que la dependencia (ApiPaciente) está caída.
        HttpStatus status = HttpStatus.SERVICE_UNAVAILABLE;

        Map<String, Object> body = new HashMap<>();
        body.put("timestamp", LocalDateTime.now());
        body.put("status", status.value());
        body.put("error", "Error de Sistema/Indisponibilidad");
        body.put("message", ex.getMessage());

        log.error("Excepción de estado ilegal ({}): {}", status, ex.getMessage());
        return new ResponseEntity<>(body, status);
    }
}
